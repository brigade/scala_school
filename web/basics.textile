...
Scala School's examples should work with Scala 2.11. This is the same version that we use for services at Brigade. Please follow the <a href="https://brigade.atlassian.net/wiki/display/EN/Data+Team+Onboarding">onboarding instructions</a> to set up Java, Scala, Maven, and Thrift.

h3. Start the Interpreter

You can start a scala console that includes all project dependencies by entering <code>mvn scala:console</code> into the command line from the root project directory.
...
If you don't have Maven set up, or don't have a maven project to work with, you can start the Scala REPL with <code>scala</code>.
...
<em>Tip:</em> If you want to copy/paste examples from these lessons into the console, enter <code>:paste</code> into the console. This will enter "Paste Mode", where you can paste your code, and do ctr-D to finish.

Alternatively, if you have IntelliJ set up, you can create a Scala Worksheet under the src/main/scala directory of your project. This has an interactive mode which will run your code live.
...
If you want to pass an iterable to such a function, you can use the splat operator <code>:_*</code>:

<pre>
scala> val params = Seq("rarity", "applejack")
params: Seq[String] = List(rarity, applejack)

scala> capitalizeAll(params: _*)
res2: Seq[String] = List(Rarity, Applejack)
</pre>
...
Note that by default all class parameters are
...
Implement a <code>Calculator</code> that overrides <code>color</code> to always return "purple". Use this definition of calculator:
