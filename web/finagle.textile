...
You can check out the <a href="https://twitter.github.io/finagle/">Finagle documentation</a> for reference.

You can spin up a Scala console that includes Finagle by including the following dependency in your project's pom.xml file:
<pre>
&lt;dependency&gt;
...
and running from the project root directory:
...
In practice, you won't write code that sends a request and then calls <code>Await.result(myFuture)</code> (or the deprecated <code>myFuture.get</code>) a few statements later. A Future has methods to register callbacks to invoke when the value becomes available.
...
pr7: com.twitter.util.Promise[Int] = Promise@1602231698(state=Waiting(null,List()))
...
<blockquote>
<code>def flatMap[B](f: (A) => Future[B]): Future[B]</code>
</blockquote>
...
pru: com.twitter.util.Promise[User] = Promise@254814841(state=Waiting(null,List()))

scala> val futBan = pru.flatMap { isBanned } // apply isBanned to future
futBan: com.twitter.util.Future[Boolean] = Promise@267993265(state=Transforming(List(),Promise@254814841(state=Waiting(<function1>,List()))))
...
scala> def normalize(raw: RawCredentials): Credentials = {
...
praw: com.twitter.util.Promise[RawCredentials] = Promise@718825575(state=Waiting(null,List()))

scala> val fcred = praw.map { normalize } // apply normalize to future
fcred: com.twitter.util.Future[Credentials] = Promise@1370659542(state=Transforming(List(),Promise@718825575(state=Waiting(<function1>,List()))))
...
<pre>
scala> case class LoginRequest(username: String)
defined class LoginRequest
...
scala> val request = LoginRequest("bob")

scala> val f = for {
     |  u <- authenticate(request)
     |  b <- isBanned(u)
     | } yield (u, b)
f: com.twitter.util.Future[(User, Boolean)] = Promise@1391415675(state=Done(Return((User@5dce8cc6,false))))
...
ready: com.twitter.util.Future[Unit] = Promise@994427881(state=Done(Return(())))
...
pr7: com.twitter.util.Promise[Int] = Promise@1911704670(state=Waiting(null,List()))

scala> val sel = Future.select(Seq(f2, pr7)) // select from 2 futs, one resolved
sel: com.twitter.util.Future[(com.twitter.util.Try[Int], Seq[com.twitter.util.Future[Int]])] = Promise@1457412623(state=Done(Return((Return(2),List(Promise@1911704670(state=Waiting(null,List())))))))

scala> val(complete, stragglers) = Await.result(sel)
complete: com.twitter.util.Try[Int] = Return(2)
stragglers: Seq[com.twitter.util.Future[Int]] = List(Promise@1911704670(state=Waiting(null,List())))

scala> Await.result(complete)
res110: Int = 2

scala> Await.result(stragglers) // our list of not-yet-finished futures has one item
...
def getTimeline(cred: Credentials): Future[Timeline] =
  isLimitedByCache(cred.user).flatMap {
    ...
    case false =>
      ...
      val limited = isRateLimited(cred.user).onSuccess { b =>
        setIsLimitedInCache(cred.user, b))
      }
...
class HTMLPage(val i: Seq[String], val l: Seq[String]) extends Resource {
  def imageLinks(): Seq[String] = i
  def links(): Seq[String] = l
}

// IMGs don't actually link to anything else
class Img extends Resource {
  def imageLinks(): Seq[String] = Seq()
...
import com.twitter.finagle.http.{Response, Request, RequestBuilder}

val client: Service[Request, Response] = Http.client.newService("localhost:80")
...
val server: ListeningServer = Http.server.serve("localhost:10000", rootService)
</pre>
...
<pre>
class MyService(client: Service[..]) extends Service[Request, Response]
{
  def apply(request: Request) = {
...
h2(#Builder). Client/Service Builders

Finagle clients and servers follow a similar pattern of construction. Starting with the protocol (i.e. <code>Http</code> or <code>Thrift</code>), you can create a client or server that takes configuration parameters and perhaps an interface.

Here's an example of an Http client (types provided for illustration):
...
Similarly, you build a service to make your service "listen" for incoming requests:
...
val server = Thrift
  ...
  .serveIface(new InetSocketAddress(port), filteredService)
</pre>

This will serve, on port <var>port</var>, a Thrift server which dispatches requests to <var>service</var>.
